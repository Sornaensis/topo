...
spec = describe "TerrainCacheWorker" $ do
  it "sends cache results to the IORef" $ withSystem $ \system -> do
    ...
    let uiSnap = emptyUiState
        terrainSnap = TerrainSnapshot 0 sampleChunkSize sampleTerrainChunks sampleClimateChunks sampleWeatherChunks mempty mempty mempty mempty emptyOverlayStore
