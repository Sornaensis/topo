...
configReadme = unlines
  ...
  , "  The directory is created and opened as a retained filesystem capability"
  , "  at startup. A relative, symlink/reparse-point, file-valued, or"
  , "  uncreatable root is an error. Saves are create-only: an existing"
  , "  destination is a conflict and is never overwritten. Linux requires"
  , "  O_TMPFILE plus linkat(AT_EMPTY_PATH); Windows uses handle-relative"
  , "  create-new publication. Configured writes fail closed when these safe"
  , "  primitives are unavailable and on other POSIX platforms. Omit this"
  , "  field or set it to null to disable filesystem writes; screenshot"
  , "  capture without a path remains available."
  ]
