...
module Topo.Plugin.Example
  ( terrainRoughenPlugin
  , main
  ) where
...
terrainRoughenPlugin = defaultPluginDef
  ...
  , pdVersion = "0.1.0"
  , pdDescription = Just "Terrain roughening generator that perturbs elevation after erosion"
  , pdRuntimeTopoMin = Just "1.0"
  , pdUiHints = defaultRPCUIHints
      { ruiDisplayName = Just "Terrain Roughen"
      , ruiCategory = Just "Generation"
      , ruiTags = ["terrain", "erosion"]
      }
  , pdParams  =
      [ ParamDef
          { paramName    = "roughness"
