...
collectSamples world =
  ...
  in concatMap (chunkSamples wc planet hex slice climate veg wb)
...
chunkSamples wc planet hex slice climate veg wb (k, tc) =
  ...
  in [ tileSample wc planet hex slice tc mcc mvc mwbc ox oy i
     | i <- [0 .. n - 1]
     ]
...
tileSample
  ...
  -> Int -> Int -> Int
  -> TileSample
tileSample wc planet hex slice tc mcc mvc mwbc ox oy i =
...
spec = beforeAll generateAndCollect $ describe "Integration (Phase 6)" $ do
  ...
  describe "6.2 Precipitation gradient" $ do
    it "coastal land tiles have mean precipitation in [0.23, 0.90]" $ \samples -> do
