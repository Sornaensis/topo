...
-- classifyTerrainForm cfg ds r1 r2 r3 c localMin 0.5 0.5 0.0
classifyDefault :: TerrainFormConfig -> DirectionalSlope -> Float -> Float -> Bool -> TerrainForm
classifyDefault cfg ds r c lm = classifyTerrainForm cfg ds r 0 0 c lm 0.5 0.5 0.0
...
spec = describe "Parameters" $ do
  ...
  describe "classifyTerrainForm" $ do
    ...
    it "classifies hilly terrain" $
      -- top3 phys = 0.08 * 0.574 = 0.046 > 0.025, relief2 = 0.06 > 0.04
      classifyTerrainForm cfg (uniformDS 0.08) 0.10 0.06 0.0 0.0 False 0.5 0.5 0.0
        `shouldBe` FormHilly
    ...
    it "classifies mountainous terrain (by relief)" $
      -- relief3 = 0.30 > 0.08 → Mountainous
      classifyTerrainForm cfg (uniformDS 0.01) 0.01 0.0 0.30 0.0 False 0.5 0.5 0.0
    ...
    -- New forms
    it "classifies ridge (steep NW/SE, flat axis)" $
      let ds = DirectionalSlope 0.0 0.0 0.13 0.0 0.0 0.13
          -- NW and SE are opposite; both steep at 0.13.  Other 4 dirs at 0.0.
          -- phys NW/SE = 0.075 ≥ ridgeMin 0.07, top3 = 0.050 < 0.06
      in classifyTerrainForm cfg ds 0.15 0 0 0.0 False 0.7 0.3 0.05
         `shouldBe` FormRidge

    it "classifies escarpment (steep one side, flat opposite)" $
      let ds = DirectionalSlope 0.20 0.0 0.0 0.02 0.0 0.0
          -- E steep (0.20 → phys 0.115 ≥ 0.085), W gentle (0.02 → phys 0.011 ≤ 0.023)
      in classifyTerrainForm cfg ds 0.10 0 0 0.0 False 0.5 0.5 0.0
         `shouldBe` FormEscarpment

    it "classifies plateau (low slope, high elevation)" $
      -- phys maxS = 0.01 * 0.574 = 0.006 ≤ 0.015, relief2 = 0 ≤ 0.03
    ...
    it "classifies badlands (steep + soft + high asymmetry)" $
      ...
      in classifyTerrainForm cfg ds 0.10 0 0 0.0 False 0.20 0.1 0.0
         `shouldBe` FormBadlands

    it "classifies pass (ridge-like + local minimum)" $
      let ds = DirectionalSlope 0.0 0.0 0.15 0.0 0.0 0.15
          -- phys NW/SE = 0.086 ≥ crossMin 0.057, axisMax = 0 ≤ 0.023
      in classifyTerrainForm cfg ds 0.10 0 0 0.0 True 0.7 0.3 0.05
         `shouldBe` FormPass

    it "classifies canyon (valley + high relief + steep walls + hard rock)" $
      let ds = DirectionalSlope 0.25 0.0 0.0 0.25 0.0 0.0
          -- phys E/W = 0.144, curv = -0.25 < -0.15, relief3 = 0.30 > 0.08
      in classifyTerrainForm cfg ds 0.30 0 0.30 (-0.25) False 0.60 0.2 0.0
         `shouldBe` FormCanyon

    it "classifies mesa (flat top + steep edges + hard cap + elevated)" $
      -- phys maxS = 0.006 ≤ 0.015, relief2 = 0.20 > 0.05
      classifyTerrainForm cfg (uniformDS 0.01) 0.01 0.20 0 0.0 False 0.55 0.3 0.10
        `shouldBe` FormMesa

    it "classifies foothill (moderate slope + moderate elevation)" $
      -- phys top3 = 0.05 * 0.574 = 0.029, in [0.015, 0.055], elev = 0.06 in [0.02, 0.12]
      classifyTerrainForm cfg (uniformDS 0.05) 0.04 0 0 0.0 False 0.5 0.5 0.06
        `shouldBe` FormFoothill

    prop "always returns a valid TerrainForm" $
      \(s :: Float) (r :: Float) (c :: Float) (lm :: Bool)
       (h :: Float) (sd :: Float) (e :: Float) ->
        let form = classifyTerrainForm cfg (uniformDS (abs s)) (abs r) 0 0 c lm
    ...
    it "mountainous beats valley" $
      ...
      classifyTerrainForm cfg (uniformDS 0.25) 0.30 0 0 (-0.20) True 0.3 0.5 0.0
        `shouldBe` FormMountainous

    it "high hardness → mountainous, not badlands" $
      -- Uniform steep slope with high hardness prevents Badlands
      classifyTerrainForm cfg (uniformDS 0.25) 0.10 0 0 0.0 False 0.60 0.1 0.0
        `shouldBe` FormMountainous

    it "moderate slope + low hardness + deep soil → hilly, not mountainous" $
      -- phys top3 = 0.08*0.574 = 0.046, relief2 = 0.06 > 0.04
      classifyTerrainForm cfg (uniformDS 0.08) 0.10 0.06 0 0.0 False 0.2 0.8 0.0
        `shouldBe` FormHilly

    -- Multi-ring relief discrimination tests
    it "moderate slope + high 3-ring relief → mountainous" $
      -- phys top3 = 0.04*0.574 = 0.023 < 0.06, but relief3 = 0.12 > 0.08
      classifyTerrainForm cfg (uniformDS 0.04) 0.05 0 0.12 0.0 False 0.5 0.5 0.0
        `shouldBe` FormMountainous

    it "moderate slope + moderate 2-ring relief → hilly" $
      -- phys top3 = 0.06*0.574 = 0.034 > 0.025, relief2 = 0.06 > 0.04
      -- Not mountainous: top3 < 0.06 and relief3 = 0 < 0.08
      classifyTerrainForm cfg (uniformDS 0.06) 0.05 0.06 0 0.0 False 0.5 0.5 0.0
        `shouldBe` FormHilly

    it "low slope + high elevation + low relief2 → plateau" $
      -- phys maxS = 0.02*0.574 = 0.011 ≤ 0.015, elev = 0.15 ≥ 0.08,
      -- relief2 = 0.01 ≤ 0.03
    ...
    it "low slope + high elevation + high relief2 → NOT plateau" $
      ...
      -- Falls through: foothill? top3=0.011<0.015=min → no.
      -- Rolling: avg=0.011>0.008 → FormRolling
      classifyTerrainForm cfg (uniformDS 0.02) 0.01 0.10 0 0.0 False 0.5 0.5 0.15
    ...
    it "zeroDirSlope with zero relief at sea level is FormFlat" $
      classifyTerrainForm cfg zeroDirSlope 0.0 0 0 0.0 False 0.5 0.5 0.0
        `shouldBe` FormFlat
    ...
    it "all 15 terrain forms are reachable" $ do
      let allForms =
          ...
          -- Witness inputs: (ds, r1, r2, r3, curvature, localMin, hardness, soil, elevASL)
          witnesses =
            [ (uniformDS 0.0,   0.0,  0, 0,    0.0,    False, 0.5,  0.5, 0.0)    -- Flat
            , (uniformDS 0.05,  0.0,  0, 0,    0.0,    False, 0.5,  0.5, 0.0)    -- Rolling
            , (uniformDS 0.08,  0.10, 0.06, 0, 0.0,    False, 0.5,  0.5, 0.0)    -- Hilly
            , (uniformDS 0.25,  0.0,  0, 0,    0.0,    False, 0.5,  0.5, 0.0)    -- Mountainous
            , (uniformDS 0.50,  0.0,  0, 0,    0.0,    False, 0.5,  0.5, 0.0)    -- Cliff
            , (uniformDS 0.01,  0.01, 0, 0,    (-0.20),False, 0.5,  0.5, 0.0)    -- Valley
            , (uniformDS 0.0,   0.0,  0, 0,    0.0,    True,  0.5,  0.5, 0.0)    -- Depression
            , (DirectionalSlope 0.0 0.0 0.13 0.0 0.0 0.13,
                                0.15, 0, 0,    0.0,    False, 0.7,  0.3, 0.05)   -- Ridge
            , (DirectionalSlope 0.20 0.0 0.0 0.02 0.0 0.0,
                                0.10, 0, 0,    0.0,    False, 0.5,  0.5, 0.0)    -- Escarpment
            , (uniformDS 0.01,  0.01, 0, 0,    0.0,    False, 0.6,  0.5, 0.15)   -- Plateau
            , (DirectionalSlope 0.25 0.01 0.01 0.01 0.01 0.01,
                                0.10, 0, 0,    0.0,    False, 0.20, 0.1, 0.0)    -- Badlands
            , (DirectionalSlope 0.0 0.0 0.15 0.0 0.0 0.15,
                                0.10, 0, 0,    0.0,    True,  0.7,  0.3, 0.05)   -- Pass
            , (DirectionalSlope 0.25 0.0 0.0 0.25 0.0 0.0,
                                0.30, 0, 0.30, (-0.25),False, 0.60, 0.2, 0.0)    -- Canyon
            , (uniformDS 0.01,  0.01, 0.20, 0, 0.0,    False, 0.55, 0.3, 0.10)   -- Mesa
            , (uniformDS 0.05,  0.04, 0, 0,    0.0,    False, 0.5,  0.5, 0.06)   -- Foothill
            ]
          results = map (\(ds,rl,r2,r3,cv,lm,h,sd,e) ->
            classifyTerrainForm cfg ds rl r2 r3 cv lm h sd e) witnesses
    ...
    let genTerrainInputs :: Gen (DirectionalSlope, Float, Float, Float, Float, Bool, Float, Float, Float)
        genTerrainInputs = oneof
          [ -- Uniform random in realistic ranges
            do e  <- choose (0.0, 0.6)
               ...
               sd  <- choose (0.0, 1.0)
               el  <- choose (-0.05, 0.25)
               pure (ds, rl, r2, r3, cv, lm, h, sd, el)
          , -- Steep / cliff zone
          ...
          , -- Canyon zone: negative curvature + high relief + steep walls + hard rock
            do wall <- choose (0.18, 0.40)
               ...
               rl <- choose (0.20, 0.50)
               r3 <- choose (0.10, 0.50)   -- relief3 > canyonMinRelief (0.08)
               cv <- choose (-0.40, -0.15)
               h  <- choose (0.40, 0.80)
               sd <- choose (0.0, 0.5)
               el <- choose (0.0, 0.1)
               pure (ds, rl, 0, r3, cv, False, h, sd, el)
          ...
          , -- Escarpment zone: steep one side, gentle opposite
            do hi <- choose (0.15, 0.35)
               ...
               sd <- choose (0.0, 0.5)
               el <- choose (0.0, 0.1)
               pure (ds, rl, 0, 0, cv, False, h, sd, el)
          ...
          , -- Plateau / Mesa zone: low slope + elevated
            do s  <- choose (0.0, 0.03)
               let ds = uniformDS s
               rl <- choose (0.0, 0.05)
               -- relief2: small → plateau; large → mesa
               r2 <- choose (0.0, 0.25)
               cv <- choose (-0.1, 0.1)
               h  <- choose (0.0, 1.0)
               sd <- choose (0.0, 1.0)
               el <- choose (0.05, 0.25)
               pure (ds, rl, r2, 0, cv, False, h, sd, el)
          , -- Foothill zone: moderate slope + moderate elevation
            do s  <- choose (0.03, 0.10)
               let ds = uniformDS s
               rl <- choose (0.0, 0.10)
               r2 <- choose (0.0, 0.04)    -- below hill relief threshold
               cv <- choose (-0.1, 0.1)
               h  <- choose (0.0, 1.0)
               sd <- choose (0.0, 1.0)
               el <- choose (0.02, 0.12)
               pure (ds, rl, r2, 0, cv, False, h, sd, el)
          , -- Hilly zone: moderate slope + relief2 > 0.04
            do s  <- choose (0.05, 0.10)
               let ds = uniformDS s
               rl <- choose (0.0, 0.15)
               r2 <- choose (0.05, 0.20)  -- above hill relief threshold
               cv <- choose (-0.1, 0.1)
               h  <- choose (0.0, 1.0)
               sd <- choose (0.0, 1.0)
               el <- choose (0.0, 0.10)
    ...
    modifyMaxSuccess (const 5000) $
      prop "no terrain form is dead — all 15 forms are produced" $
        forAll genTerrainInputs $ \(ds, rl, r2, r3, cv, lm, h, sd, el) ->
          let form = classifyTerrainForm cfg ds rl r2 r3 cv lm h sd el
    ...
    it "mesa is not always shadowed by plateau" $
      ...
      -- With high edge relief2 and hard cap, mesa should win.
      classifyTerrainForm cfg (uniformDS 0.01) 0.01 0.20 0 0.0 False 0.55 0.3 0.10
        `shouldBe` FormMesa
    ...
    it "badlands is not always shadowed by mountainous" $
      -- Badlands fires before mountainous when: phys maxSlope >= 0.085,
      -- hardness <= 0.35, asymmetry >= 0.035.  top3 stays < 0.06.
      let ds = DirectionalSlope 0.18 0.01 0.01 0.01 0.01 0.01
      in classifyTerrainForm cfg ds 0.05 0 0 0.0 False 0.20 0.1 0.0
         `shouldBe` FormBadlands

    it "canyon is not always shadowed by cliff" $
      -- Canyon requires steep walls (phys 0.115) but not cliff-level (0.15).
      -- With relief3=0.30 > 0.08, canyon fires.
    ...
    it "depression is not always shadowed by valley" $
      ...
      classifyTerrainForm cfg (uniformDS 0.0) 0.0 0 0 0.0 True 0.5 0.5 0.0
        `shouldBe` FormDepression

    it "hilly is not always shadowed by foothill" $
      -- Hilly: phys top3 > 0.025, relief2 > 0.04.  Foothill requires
      -- phys top3 in [0.015, 0.055] and elevASL in [0.02, 0.12].
      -- With top3=0.046 (above foothill max 0.055? no: 0.046<0.055)
      -- So use top3 above foothill max: 0.10*0.574=0.057>0.055, relief2=0.06>0.04
      classifyTerrainForm cfg (uniformDS 0.10) 0.10 0.06 0 0.0 False 0.5 0.5 0.0
        `shouldBe` FormHilly
