...
| Command IPC, dispatch, and UI action compatibility adapters | Compatibility | topo-seer service maintainers | Current UI and internal/test command IPC compatibility path. | AppService backs compatibility adapters; HTTP/OpenAPI replaces command IPC as public automation. Legacy command IPC has no public 1.0 exception. | Existing command and HTTP route parity tests. |
